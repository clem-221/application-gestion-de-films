...
def movies():
    ...
    return render_template("movies_view.html", movies=all_movies,
                            metadata={"title": "Movies", "pagename": "movies"})
...
def movies_by_year(year):
    ...
    if not movies:
        abort(404)
    return render_template("movies_view.html", year=year, movies=movies,
                            metadata={"title": f"Movies {year}", "pagename": "movies"})
